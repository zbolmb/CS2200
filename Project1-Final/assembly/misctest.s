...
test1:  LEA $k0, test1                      ! test #1 (if you fail, check $k0)
        ADDI $zero, $zero, 5                ! try to add 5 to $zero
        ADDI $s0, $zero, 0                  ! put $zero into $s0 to see if $zero changed
        BRnp fail
        LEA $at, compare
test2:  LEA $k0, test2                      ! test #2 (if you fail, check $k0)
...
test3:  LEA $k0, test3                      ! test #3 (if you fail, check $k0)
        ...
        JALR $at, $ra
        BRnp fail                           ! fail if not equal
test4:  LEA $k0, test4                      ! test #4 (if you fail, check $k0)
        ...
        JALR $at, $ra
        BRnp fail                           ! fail if not equal
success:LEA $k0, success                    ! clear error number
        HALT                                ! Yay! we have passed
compare:NAND $a1, $a1, $a1                  ! take 2's complement of $a1
        ADDI $a1, $a1, 1                    ! (NOT $a1) + 1
        ADD $v0, $a0, $a1                   ! $v0 = $a0 - $a1
        RET
fail:   ADDI $t0, $zero, -1                 ! set $t registers to signal error
        ADDI $t1, $zero, -1
        ADDI $t2, $zero, -1
        HALT
